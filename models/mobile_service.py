...
class MobileService(models.Model):
    ...
    city = fields.Char( string="Address",
                       help="City of the customer.")
    state_id = fields.Char(string="Address",
                               help="State of the customer.")
    zip = fields.Char(string="Address",
    ...
    return_date = fields.Date(string="Return Date", help="Device returned date.")
    ...
    @api.depends('image_medium', 'person_name','service_state')
    def _compute_display_name_after_upload(self):
        for rec in self:
            # Show Mr./Miss Name if state is assigned or later
            if rec.service_state in ['assigned', 'completed', 'not_solved', 'returned']:
                prefix = ''
                rec.display_name_after_upload = f"{prefix}{rec.person_name.name if rec.person_name else ''}"
            else:
                rec.display_name_after_upload = False
    ...
    total_profit = fields.Float(string='Total Profit', compute='_compute_total_profit',
                                store=True, help='Total profit for this service (sum of part profits)')
    ...
    @api.depends('product_order_line.profit_price')
    def _compute_total_profit(self):
        """Sum the profit_price of all product_order_line lines for this service."""
        for rec in self:
            total = 0.0
            for line in rec.product_order_line:
                total += (line.profit_price or 0.0)
